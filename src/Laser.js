// Constants for laser size
const laserWidth = 12;
const laserHeight = 12;

// Class to represent a laser shot by the player
function Laser(laserState) {
  this.id = laserState.id;
  this.playerId = laserState.playerId;

  // Initially set position at the centre of player div
  this.position = {
    x: laserState.pos.x - laserWidth / 2, 
    y: laserState.pos.y - laserHeight / 2
  };

  // Calculate x and y components of velocity using angle
  this.velocity = {
    x: Math.cos(laserState.angle - Math.PI / 2) * 8, 
    y: Math.sin(laserState.angle - Math.PI / 2) * 8
  };
